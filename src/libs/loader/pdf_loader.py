"""PDF Loader implementation with fallback support.
...
Features:
- Text extraction and Markdown conversion via MarkItDown (preferred) or pypdf (fallback)
- Image extraction and storage (via PyMuPDF if available)
...
class PdfLoader(BaseLoader):
    ...
    def __init__(
        ...
        image_storage_dir: Union[str, Path] = "data/images"
    ):
        ...
        """
        if not MARKITDOWN_AVAILABLE and not PYPDF_AVAILABLE:
            raise ImportError(
                "Either MarkItDown or pypdf is required for PdfLoader. "
                "Install with: pip install markitdown  OR  pip install pypdf"
            )
    ...
    def load(self, file_path: Union[str, Path]) -> Document:
        ...
        # Parse PDF with MarkItDown or pypdf fallback
        try:
