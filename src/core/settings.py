...
Environment variables are loaded automatically from a .env file in the repo root
(if present) before YAML parsing. Supports ${VAR} and ${VAR:default} syntax.
"""
...
def _load_dotenv() -> None:
    """Load variables from .env file in repo root (if it exists).

    Mirrors python-dotenv behaviour but without requiring an extra dependency.
...
def _expand_env_vars(data: Any) -> Any:
    """Recursively expand ${ENV_VAR} or ${ENV_VAR:default} in a data structure.
